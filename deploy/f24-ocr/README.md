...
emails even though all emails go to `angela@elaborazionistudio.com` in To and
`mr10dev10@gmail.com` and `mohamedelhaddad997@gmail.com` in BCC. Client
...
`invoice_number` and ISO `due_date` for each invoice. The configured recipients are
`angela@elaborazionistudio.com` (To), with `mr10dev10@gmail.com` and
`mohamedelhaddad997@gmail.com` in BCC.
The API's `emails` field lists only To recipients.
